...
// diberi spasi fungsinya untuk menghapus	
 string kalimat=" dwi sakethi ilmu komputer fmipa unila ";
...
for (huruf=0;huruf<perulangan;huruf++)
{
    ...
    *------------------------------------------- */  
    ...
    kiri = kalimat.substr(0,perulangan-huruf+1);
    //strcat(kiri,kalimat[huruf]);
   
    kanan = kalimat.substr(perulangan+huruf-1,perulangan-huruf);

   /* ------------------------------------------
    * pada posisi sekarang (baris),  
    * cetak karakter saat ini
    *------------------------------------------- */  
    mvprintw(baris,kolom+huruf,"%s%s",kiri.c_str(),kanan.c_str());
